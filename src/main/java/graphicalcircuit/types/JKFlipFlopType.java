...
import circuit.JKFlipFlop;
import configuration.Table;

public class JKFlipFlopType extends AbstractCircuitType
{
  ...
  public JKFlipFlopType()
  {
    int WIDTH = JKFlipFlopType.WIDTH + PIN_LENGHT*2;
  ...
  public CircuitType makeCopy() {
    return new JKFlipFlopType();
  }
  
  @Override
  public Component makeBackendComponent(int id) {
    return new JKFlipFlop(id);
  }
  ...
  protected void renderType(Graphics2D g, int x, int y, Component component, ComponentConfig config)
  {
    ...
    g.drawString(
        "K", 
        x + AbstractCircuitType.PIN_LENGHT + 2 + inputs[2].getPosition().x, 
        y + inputs[2].getPosition().y + m.getAscent()/2);
  ...
  {
    return "JK " + Table.get("circuit_flip_flop");
  }
}
