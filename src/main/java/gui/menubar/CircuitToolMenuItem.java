...
public class CircuitToolMenuItem extends AbstractMenuItem implements Observer
{
  public CircuitToolMenuItem(ProgramUI window)
  {
    super(Table.get("menubar_circuit_tool"), window);
  ...
  {
    program.getBackend().getSession().getClient().getWorkspace().tool = ToolFactory.getTool(ToolFactory.Type.PLACE_CIRCUIT);
    program.repaint();
  }
}
